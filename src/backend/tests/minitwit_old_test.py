...
class Old_Minitwit_GUI_Tests(unittest.TestCase):
    ...
    # helper functions
    async def register(self, username, password, password2=None, email=None):
    ...
    async def logout(self):
        ...
        return await client.get('/logout', allow_redirects=True)
    ...
    @patch.object(Database, "connect_db")
    async def test_timelines(self, connect_db_mock):
        with self.override_get_db() as mocK_return:
            connect_db_mock.return_value = mocK_return
            ...
            rv = await client.get('/api/users/foo/follow', allow_redirects=True)
            ...
            rv = await client.get('/timeline/bar')
            assert 'the message by foo' not in rv.text
            assert 'the message by bar' in rv.text
            rv = await client.get('/timeline/foo')
            assert 'the message by foo' in rv.text
            assert 'the message by bar' not in rv.text

            # now unfollow and check if that worked
            rv = await client.get('/api/users/foo/unfollow', allow_redirects=True)
